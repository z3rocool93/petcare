...
{{-- Sticky Footer: flex-col y min-h-screen en el body --}}
...
<header class="flex-grow">
    <nav class="bg-white border-gray-200 dark:bg-gray-900 border-b">
        <div class="max-w-screen-xl mx-auto p-4 flex items-center justify-between">
            ...
            {{-- 1. LADO IZQUIERDO: Logo (1/4 del ancho en escritorio) --}}
            ...
            {{-- 2. CENTRO: Menú (Ocupa el espacio central y se centra) --}}
            {{-- En móvil se oculta y en escritorio se centra con justify-center --}}
            <div class="hidden md:flex md:w-2/4 justify-center">
                <ul class="flex space-x-8 font-medium">
                    <li><a href="#" class="text-blue-700 dark:text-blue-500">Inicio</a></li>
                    <li><a href="{{ route('pets.index') }}" class="text-gray-900 dark:text-white hover:text-blue-700">Mascotas</a></li>
                    <li><a href="{{ route('forum.index') }}" class="text-gray-900 dark:text-white hover:text-blue-700">Foro</a></li>
                    <li><a href="{{ route('membership.index') }}" class="text-gray-900 dark:text-white hover:text-blue-700">Planes</a></li>
                </ul>
            </div>

            {{-- 3. LADO DERECHO: Auth y Toggle (1/4 del ancho, alineado al final) --}}
            <div class="flex items-center space-x-2 md:w-1/4 justify-end">
                @if (Route::has('login'))
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-gray-800 dark:text-white hover:bg-gray-50 font-medium rounded-lg text-sm px-3 py-2">Log in</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600">Registrarse</a>
                        @endif
                    @endauth
                @endif
                ...
                <button @click="mobileMenuOpen = !mobileMenuOpen" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100">
        ...
        {{-- Menú Mobile (se despliega abajo) --}}
        <div x-show="mobileMenuOpen" class="md:hidden bg-gray-50 dark:bg-gray-800 border-b dark:border-gray-700">
...
{{-- Footer: Pegado al final con mt-auto --}}
